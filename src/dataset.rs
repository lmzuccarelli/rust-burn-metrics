...
// Inputs are cpu,memory,disk,qtime
const FEATURES_MIN: [f32; NUM_FEATURES] = [0.0, 0.0, 0.0, 0.0];
const FEATURES_MAX: [f32; NUM_FEATURES] = [100.0, 100.0, 100.0, 100.0];
...
impl MetricsDataset {
    ...
    pub fn new(split: &str) -> Self {
        ...
        let file_name = match split.as_ref() {
            "train" => "data/metrics-100000.csv",
            "validation" => "data/metrics-20000.csv",
            "test" => "data/metrics-1000.csv",
            _ => "data/metrics-100000.csv",
        };
...
impl Dataset<Metrics> for MetricsDataset {
    fn get(&self, index: usize) -> Option<Metrics> {
...
#[derive(Clone, Debug)]
pub struct MetricsBatcher<B: Backend> {
    device: B::Device,
    normalizer: Normalizer<B>,
}

#[derive(Clone, Debug)]
pub struct MetricsBatch<B: Backend> {
    pub inputs: Tensor<B, 2>,
    pub targets: Tensor<B, 1, Int>,
}

impl<B: Backend> MetricsBatcher<B> {
    ...
    fn batch(&self, items: Vec<Metrics>, device: &B::Device) -> MetricsBatch<B> {
        ...
        for item in items.iter() {
            let input_tensor = Tensor::<B, 1>::from_floats(
                [item.cpu, item.memory, item.disk, item.qtime],
                &self.device,
            );
