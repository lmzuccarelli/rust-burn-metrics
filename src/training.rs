use crate::dataset::{MetricsBatcher, MetricsDataset, NUM_CLASSES, NUM_FEATURES};
...
pub fn run<B: AutodiffBackend>(artifact_dir: &str, device: B::Device) {
    ...
    let train_dataset = MetricsDataset::train();
    let valid_dataset = MetricsDataset::validation();

    println!("Train Dataset Size: {}", train_dataset.len());
    println!("Valid Dataset Size: {}", valid_dataset.len());

    let batcher_train = MetricsBatcher::<B>::new(device.clone());
    let batcher_validate = MetricsBatcher::<B::InnerBackend>::new(device.clone());
