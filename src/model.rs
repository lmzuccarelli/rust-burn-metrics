use crate::dataset::MetricsBatch;
use burn::{
...
impl<B: AutodiffBackend> TrainStep<MetricsBatch<B>, ClassificationOutput<B>> for Model<B> {
    fn step(&self, item: MetricsBatch<B>) -> TrainOutput<ClassificationOutput<B>> {
        let item = self.forward_classification(item.inputs, item.targets);
        TrainOutput::new(self, item.loss.backward(), item)
    }
}

impl<B: Backend> ValidStep<MetricsBatch<B>, ClassificationOutput<B>> for Model<B> {
    fn step(&self, item: MetricsBatch<B>) -> ClassificationOutput<B> {
